...
#+CAPTION: Sequences
|       Slots | Mondays          | Tuesday          | Wednesday        | Thursday         | Friday           |
|-------------+------------------+------------------+------------------+------------------+------------------|
| 08:00-09:40 |                  |                  |                  |                  |                  |
| 10:20-12:00 |                  | [2016-03-29 Tue] |                  | [2016-03-31 Thu] | _[2016-04-01 Fri]_ |
| 13:30-17:30 |                  | [2016-03-29 Tue] |                  | [2016-03-31 Thu] | [2016-04-01 Fri] |
|-------------+------------------+------------------+------------------+------------------+------------------|
| 08:00-09:40 |                  |                  |                  |                  |                  |
| 10:20-12:00 |                  | _[2016-04-05 Tue]_ | _[2016-04-06 Wed]_ |                  |                  |
| 13:30-17:30 |                  | [2016-04-05 Tue] |                  |                  |                  |
|-------------+------------------+------------------+------------------+------------------+------------------|
| 08:00-09:40 |                  | [2016-04-12 Tue] |                  |                  |                  |
| 10:20-12:00 | [2016-04-11 Mon] | [2016-04-12 Tue] |                  |                  |                  |
| 13:30-17:30 |                  |                  |                  |                  |                  |
|-------------+------------------+------------------+------------------+------------------+------------------|
** During the labworks
...
The three previous  /OTB-Applications/ are available from the command line interface (CLI):
...
dates, so the  total number of couples is  src_calc{13*12/2}. I really
...
* Appendix                                                            :sigma:
** Introduction to shell <<sec:shell>>
...
#+BEGIN_SRC sh :exports both :session s8
...
#+BEGIN_SRC sh :exports both :session s8 :results code
...
#+BEGIN_SRC sh :exports code :tangle script.sh
...
#+BEGIN_SRC sh :exports none
# TrainClassifier
otbcli_TrainImagesClassifier -io.il fabas_12_10_2013.tif -io.vd train_fabas.shp -io.out model_knn -classifier knn

# Predict
otbcli_ImageClassifier -in fabas_12_10_2013.tif -model model_knn -out classif_knn.tif

# Confusion
otbcli_ComputeConfusionMatrix -in classif_knn.tif -out confu_knn.csv -ref vector -ref.vector.in valid_fabas.shp
#+END_SRC
...
url = 'https://ethercalc.org/fauvel_rs_classification_2'
...
for i in range(4):
    ...
    plt.figure()
    plt.bar(index,m[i*4+1:i*4+5],bar_width,alpha=opacity,color='b',yerr=s[i*4+1:i*4+5],error_kw=error_config)
...
#+NAME: Addition
#+BEGIN_SRC python :var tab=tab :results output :exports none
import matplotlib.pyplot as plt
...
NAME_TRAIN,NAME_VALID = glob.glob('confu_train_*.csv'),glob.glob('confu_valid_*.csv')
NAME_TRAIN.sort()
NAME_VALID.sort()

oa_train,oa_valid = [],[]

for name_train,name_valid in zip(NAME_TRAIN,NAME_VALID):
...
#+BEGIN_SRC sh :exports none
# Iteration on the max number of training samples
for classifier in bayes rf knn
do
    # do repetition
    for rep in `seq 1 20`
    do
	# trainclassifier
	otbcli_TrainImagesClassifier -io.il fabas_12_10_2013.tif -io.vd train_fabas.shp -io.out model -classifier ${classifier} -sample.mt 500
	
	# Predict
	otbcli_ImageClassifier -in fabas_12_10_2013.tif -model model -out classif.tif
	
	# Confusion on the training set
	otbcli_ComputeConfusionMatrix -in classif.tif -out confu_train_${rep}.csv -ref vector -ref.vector.in train_fabas.shp
	
	# Confusion on the validation set
	otbcli_ComputeConfusionMatrix -in classif.tif -out confu_valid_${rep}.csv -ref vector -ref.vector.in valid_fabas.shp
    done
    python compute_accuracy.py # Compute the accuracy
    mv acc.csv acc_${classifier}.csv
    rm confu_*.csv model classif.tif # Cleaning
done
...
| ITEM                                                             | FORMATION      | DURATION | SEQUENCE | DAYS                         |
|------------------------------------------------------------------+----------------+----------+----------+------------------------------|
...
|------------------------------------------------------------------+----------------+----------+----------+------------------------------|
| * Change detection: /Detection of floods/                        | Presential     |    01:00 |        2 | [2016-03-31 Thu 10:20-12:00] |
|------------------------------------------------------------------+----------------+----------+----------+------------------------------|
| * Classification of remote sensing images                        | Presential     |    04:00 |        3 | [2016-03-31 Thu 13:30-17:30] |
|------------------------------------------------------------------+----------------+----------+----------+------------------------------|
| ** Influence of the spatial distribution of the learning samples | Non Presential |    01:40 |        4 | [2016-04-01 Fri 10:20-12:00] |
|------------------------------------------------------------------+----------------+----------+----------+------------------------------|
| * Satellite Image Time Series                                    | Presential     |    04:00 |        5 | [2016-04-01 Fri 13:30-17:30] |
|------------------------------------------------------------------+----------------+----------+----------+------------------------------|
| ** Extraction of the best couple of dates                        | Non Presential |    01:40 |        6 | [2016-04-06 Wed 10:20-12:00] |
|------------------------------------------------------------------+----------------+----------+----------+------------------------------|
| * Dynamic Habitat Index                                          | Presential     |    04:00 |        7 | [2016-04-05 Tue 13:30-17:30] |
#+END:
