...
Thanks to the Processing ToolBox, it is now available to use your algorithm with
others already implemented in Qgis. A Graphical User Interface (GUI) is
automatically created, so there are some constraints to respect in order to be
compatible with Qgis.

1. Plugin Builder in Qgis
Plugin Builder is a plugin in Qgis which allows you to having some help to
...
folder as other already installed plugins :
...
ToolBox, the easiest way to find 
...
#+BEGIN_SRC shell
ls #list of files and folder in YourPluginClassName
...
#+BEGIN_work
Make your own Qgis plugin and try to use it with others in the graphical modeler.
#+END_work
